...
def _skip(reason: str) -> None:
    print(f"anthropic real chat skipped: {reason}")
    pytest.skip(reason)
...
def _real_config() -> ProviderConfig:
    ...
    if not api_key:
        _skip("ANTHROPIC_API_KEY is required")
    if not model:
        _skip("ANTHROPIC_MODEL is required")
...
async def _run_real_chat() -> None:
    ...
    try:
        ...
        print()
        print("anthropic real chat response:")
        print(f"  model: {response.model}")
        print(f"  finish_reason: {response.finish_reason}")
        print(f"  usage: {response.usage}")
        print(f"  text: {response.message.content[0].text}")
    except ProviderError as exc:
        _skip(f"configured endpoint rejected the Anthropic request: {exc}")
    finally:
